...
function calculateBonusByProfit(index, total, seller) {
  ...
  // Расчет бонуса в зависимости от позиции в рейтинге
  // Индекс начинается с 0 (первое место = index 0)
  if (index === 0) {
    // 15% для продавца на первом месте (наибольшая прибыль)
    return profit * 0.15;
...
function analyzeSalesData(data, options) {
  ...
  const sellerIndex = Object.fromEntries(
    sellerStats.map((stats) => [stats.seller_id, stats])
  );
